...
let rec explicate_tail (e : L.exp) : tail =
  match e with
  | L.Atm a -> Return (Atm (convert_atom a))
  | L.Read -> Return Read
  | L.Negate a -> Return (Negate (convert_atom a))
  | L.Add (a1, a2) -> Return (Add (convert_atom a1, convert_atom a2))
  | L.Sub (a1, a2) -> Return (Sub (convert_atom a1, convert_atom a2))
