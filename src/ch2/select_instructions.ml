...
let process_exp exp =
  ...
  | C.Read -> [ Callq (Label (fix_label "read_int"), 0) ]
  | C.Negate atm -> [ Movq (process_atm atm, Reg Rax); Negq (Reg Rax) ]
  | C.Add (atm1, atm2) ->
...
let convert_lt (lt : Types.label * C.tail) : Types.label * block =
  match lt with label, tail -> (label, Block (tail_instructions tail))
