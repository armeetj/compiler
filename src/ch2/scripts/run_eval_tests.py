...
def run_assembly_program(progname, inputs, output, arm64=False):
    """
...
def run_eval_program_all(progname):
    ...
    for (i, (inputs, output)) in enumerate(progdata):
        ...
        if compile_asm:
            run_assembly_program(progname, inputs, output, compile_arm64)
        if verbose:
            print()
