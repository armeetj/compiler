...
let make_graph (lbs : (label * 'a block) list) : LabelDgraph.t =
  let rec add_vertices lbs g =
    match lbs with
    | [] -> g
    | (l, _) :: t -> add_vertices t (if l <> Label "conclusion" then G.add_vertex g l else g)
  in
  let rec add_edges lbs g =
    ...
    | (l1, b1) :: t ->
        let rec add_edges_from_label labels g =
          match labels with
          | [] -> g
          | l2 :: t -> add_edges_from_label t (if l2 <> Label "conclusion" then G.add_edge g l1 l2 else g)
        in
...
let get_mergeable_labels (g : G.t) : (label * label) option =
  let rec find_mergeable vs = 
    match vs with
    | [] -> None
    | l :: rest -> match G.neighbors_out g l with
      | [ele] -> 
