...
let x86_var_if1_in =
  X86_var_if.program_of_sexp X86_var_if.info1_of_sexp X86_var_if.binfo1_of_sexp

let x86_var_if2_in =
  X86_var_if.program_of_sexp X86_var_if.info1_of_sexp X86_var_if.binfo2_of_sexp

let x86_var_if3_in =
  X86_var_if.program_of_sexp X86_var_if.info2_of_sexp X86_var_if.binfo1_of_sexp

let x86_var_if4_in =
  X86_var_if.program_of_sexp X86_var_if.info3_of_sexp X86_var_if.binfo1_of_sexp
...
let x86_var_if1_out =
  X86_var_if.sexp_of_program X86_var_if.sexp_of_info1 X86_var_if.sexp_of_binfo1

let x86_var_if2_out =
  X86_var_if.sexp_of_program X86_var_if.sexp_of_info1 X86_var_if.sexp_of_binfo2

let x86_var_if3_out =
  X86_var_if.sexp_of_program X86_var_if.sexp_of_info2 X86_var_if.sexp_of_binfo1

let x86_var_if4_out =
  X86_var_if.sexp_of_program X86_var_if.sexp_of_info3 X86_var_if.sexp_of_binfo1
...
let eval_alist =
  ...
  ; ( "ru"
    , fun exp -> exp |> tc1 |> sh |> un |> rc |> ec |> tc2 |> ru |> cif_eval )
  ]
...
let pass_alist =
  ...
  ; ("ru", fun exp -> exp |> tc1 |> sh |> un |> rc |> ec |> tc2 |> ru |> cif_out)
  ; ( "si"
    , fun exp ->
        exp |> tc1 |> sh |> un |> rc |> ec |> tc2 |> ru |> si |> x86_var_if1_out
    )
  ; ( "ul"
    , fun exp ->
...
let run_pass pass filename =
  ...
  | None ->
      if pass = "pa" then
        read_lif filename |> tc1 |> sh |> un |> rc |> ec |> tc2 |> ru |> si
        |> ul |> bi |> ar |> rj |> pi |> pc |> pa |> print_string
      else (
        Printf.eprintf "invalid pass: %s\n" pass ;
        exit 1 )
  | Some f ->
      read_lif filename |> f |> print_sexp
