...
let uncover_live_in_block
  ...
  let l_before instr (l_after : LocSet.t) =
    ...
    | X.Jmp label -> 
      ...
      ) (LocSet.elements (LabelMap.find label live_before_map)) in
      read_set args l_after
    | X.Xorq (arg1, arg2) -> read_set [arg1; arg2] l_after
    | X.Cmpq (arg1, arg2) -> read_set [arg1; arg2] l_after
    | X.Set (_, arg) -> write_set [arg] l_after
    | X.Movzbq (arg1, arg2) -> read_set [arg1] (write_set [arg2] l_after)
    | _ -> failwith "JmpIf not implemented"
  in
  (* takes in first instruction in instructions and adds L_before to acc*)
  let process instr acc = match acc with (*returns acc with new value*)
  | h :: _ -> (l_before instr h) :: acc
  | _ -> failwith "Shouldn't have this case"
  in
  ...
  let rec helper instrs_left acc = 
    ...
    | [] -> acc
    | instr :: tail -> helper tail (process instr acc)
  (* reverse instructions to allow recurrence L_after(k) = L_before(k+1)*)
  in match helper (List.rev instrs) [LocSet.empty] with
...
let get_next_labels (block : 'a X.block) : LabelSet.t =
  ...
  let rec helper instrs labels =
    ...
    | instr :: t ->
        match instr with
        | X.Jmp label
        | X.JmpIf (_, label) -> helper t (LabelSet.add label labels)
        | _ -> helper t labels
  in
