...
let rec ltup_exp_of_sexp = function
  | List [Atom "set!"; Atom v; e] -> SetBang (v, ltup_exp_of_sexp e)
  | List (Atom "begin" :: es) -> (
  ...
  | List [Atom "and"; e1; e2] -> And (ltup_exp_of_sexp e1, ltup_exp_of_sexp e2)
  | List [Atom "or"; e1; e2] -> Or (ltup_exp_of_sexp e1, ltup_exp_of_sexp e2)
  | List (Atom "vector" :: es) -> Vec (List.map ltup_exp_of_sexp es, None)
  | List [Atom "vector-length"; e] -> VecLen (ltup_exp_of_sexp e)
  ...
  | List [Atom "-"; x] -> Prim (`Negate, [ltup_exp_of_sexp x])
  | List (Atom a :: lst) -> (
    match List.assoc_opt a ops with
    | Some op -> Prim (op, List.map ltup_exp_of_sexp lst)
    | None -> failwithf "invalid `Ltup` input: invalid operator: %s" a )
  | Atom s -> (
    ...
    | None -> (
      ...
      | Some b -> Bool b )
    | Some i -> Int i )
  | sx -> failwithf "invalid `Ltup` input: [ %s ]" (Sexp.to_string sx)
