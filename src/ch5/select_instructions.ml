...
let process_exp (exp : C.exp) : instr list =
  ...
  | C.Prim (cop, al) -> match cop, al with
    | `Read, [] -> [ Callq (Label (fix_label "read_int"), 0) ]
    | `Add, [atm1; atm2] -> [ Movq (process_atm atm1, Reg Rax); Addq (process_atm atm2, Reg Rax) ]
    | `Sub, [atm1; atm2] -> [ Movq (process_atm atm1, Reg Rax); Subq (process_atm atm2, Reg Rax) ]
...
let process_stmt stmt : instr list =
  match stmt with
  | C.PrimS (stmt_op, atm_lst) -> (match stmt_op, atm_lst with 
    | `Read, [] -> [ Callq (Label (fix_label "read_int"), 0) ]
    | `Print, [arg] -> [ 
  ...
  | C.Assign (v, C.Prim (cop, al)) -> match cop, al with
    | `Read, [] -> [ Callq (Label (fix_label "read_int"), 0); Movq (Reg Rax, Var v) ]
    | `Add, [atm1; atm2] -> [ Movq (process_atm atm1, Var v); Addq (process_atm atm2, Var v) ]
    | `Sub, [atm1; atm2] -> [ Movq (process_atm atm1, Var v); Subq (process_atm atm2, Var v) ]
