...
let collect_set_vars (e : L.exp) : S.t =
  let rec helper s_acc e =
    match e with
    | L.Prim (_, exp_lst) -> List.fold_left (fun acc exp -> helper acc exp) s_acc exp_lst
...
let rec uncover_get_exp (s : S.t) (e : L.exp) : exp =
  ...
  | L.Prim (c_op, exp_lst) -> Prim (c_op, (List.map (fun exp -> uncover_get_exp s exp) exp_lst))
  | L.SetBang (c_op, exp) -> SetBang (c_op, uncover_get_exp s exp)
  | L.Begin (exp_lst, exp) -> Begin ((List.map (fun exp -> uncover_get_exp s exp) exp_lst), uncover_get_exp s exp)
  | L.If (e1, e2, e3) -> If (uncover_get_exp s e1, uncover_get_exp s e2, uncover_get_exp s e3)
