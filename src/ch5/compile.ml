...
let x86_var_loop1_in =
  X86_var_loop.program_of_sexp X86_var_loop.info1_of_sexp
    X86_var_loop.binfo1_of_sexp

let x86_var_loop2_in =
  X86_var_loop.program_of_sexp X86_var_loop.info1_of_sexp
    X86_var_loop.binfo2_of_sexp

let x86_var_loop3_in =
  X86_var_loop.program_of_sexp X86_var_loop.info2_of_sexp
    X86_var_loop.binfo1_of_sexp

let x86_var_loop4_in =
  X86_var_loop.program_of_sexp X86_var_loop.info3_of_sexp
    X86_var_loop.binfo1_of_sexp
...
let x86_var_loop1_out =
  X86_var_loop.sexp_of_program X86_var_loop.sexp_of_info1
    X86_var_loop.sexp_of_binfo1

let x86_var_loop2_out =
  X86_var_loop.sexp_of_program X86_var_loop.sexp_of_info1
    X86_var_loop.sexp_of_binfo2

let x86_var_loop3_out =
  X86_var_loop.sexp_of_program X86_var_loop.sexp_of_info2
    X86_var_loop.sexp_of_binfo1

let x86_var_loop4_out =
  X86_var_loop.sexp_of_program X86_var_loop.sexp_of_info3
    X86_var_loop.sexp_of_binfo1
...
let eval_alist =
  ...
  ; ( "tc2"
    , fun exp -> exp |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> cloop_eval
    )
  ; ( "ru"
    , fun exp ->
...
let run_evaluator pass filename =
  ...
  | None ->
      if List.mem pass passes then (
        ...
        exit 1 )
  | Some f ->
      read_lwhile filename |> f |> print_int
...
let pass_alist =
  ...
  ; ( "tc2"
    , fun exp -> exp |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> cloop_out )
  ; ( "ru"
    , fun exp ->
  ...
  ; ( "ar"
    , fun exp ->
        exp |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> ru |> si |> ul |> bi
        |> ar |> x86_var_loop4_out )
  ; ( "rj"
    , fun exp ->
        exp |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> ru |> si |> ul |> bi
        |> ar |> rj |> x86_var_loop4_out )
  ; ( "pi"
    , fun exp ->
        exp |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> ru |> si |> ul |> bi
        |> ar |> rj |> pi |> x86_loop_out )
  ; ( "pc"
    , fun exp ->
        exp |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> ru |> si |> ul |> bi
        |> ar |> rj |> pi |> pc |> x86_asm_out ) ]
...
let run_pass pass filename =
  ...
  | None ->
      if pass = "pa" then
        read_lwhile filename |> tc1 |> sh |> un |> ug |> rc |> ec |> tc2 |> ru
        |> si |> ul |> bi |> ar |> rj |> pi |> pc |> pa |> print_string
      else (
        Printf.eprintf "invalid pass: %s\n" pass ;
        exit 1 )
  | Some f ->
      read_lwhile filename |> f |> print_sexp
