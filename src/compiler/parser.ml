...
let rec lfun_exp_of_sexp = function
  ...
  | List [Atom "and"; e1; e2] -> And (lfun_exp_of_sexp e1, lfun_exp_of_sexp e2)
  | List [Atom "or"; e1; e2] -> Or (lfun_exp_of_sexp e1, lfun_exp_of_sexp e2)
  | List (Atom "vector" :: es) -> Vec (List.map lfun_exp_of_sexp es, None)
  | List [Atom "vector-length"; e] -> VecLen (lfun_exp_of_sexp e)
  ...
  | List [Atom "vector-set!"; e1; Atom s; e2] -> (
    ...
    | None ->
      failwithf "%s: `vector-set!` second argument should be a literal integer"
        "invalid `Lfun` input" )
  ...
  | List (e1 :: es) -> Apply (lfun_exp_of_sexp e1, List.map lfun_exp_of_sexp es)
  | Atom s -> (
    match s with
    | "#f" -> Bool false
    | "#t" -> Bool true
...
let rec lfun_type_of_sexp sx =
  ...
  | List ss ->
    ...
    else failwithf "invalid type: [ %s ]" (Sexp.to_string sx)
...
let lfun_arg_of_sexp sx =
  ...
  | _ ->
    failwithf "invalid function formal argument: [ %s ]" (Sexp.to_string sx)

let lfun_def_of_sexp sx =
  match sx with
  | List [Atom "define"; List (Atom name :: sx_args); Atom ":"; sx_type; sx_exp]
    ->
...
let parse filename = parse_from_sexps (load_sexps filename)
