...
let type_check_call (ty_env : ty_env_t) (f : atm) (args : atm list) : ty option
    =
  ...
  | Some ty -> (
    ...
    | Function (arg_tys', ret_ty) ->
      if List.length arg_tys = List.length args then
        ...
        if arg_tys = arg_tys' then Some ret_ty
        else ty_err_simple "type_check_call: argument type mismatch"
      else
        (* Can't do a full type check, so just use return type. *)
        Some ret_ty
    | _ -> ty_err_simple "type_check_call: non-function in function position" )

let type_check_exp (ty_env : ty_env_t) (e : exp) : ty option =
  match e with
  | Atm a -> type_check_atm ty_env a
  | Prim (op, atms) ->
  ...
  | GlobalVal v -> (
    ...
    | _ ->
      failwithf "type_check_exp: global-val : unknown global variable (%s)" v )
  | VecLen a -> (
    ...
    | Some t ->
      ty_err_simple
      @@ Printf.sprintf
           ("type_check_exp: " ^^ "vector-length : expected a vector, got: %s")
           (string_of_ty t) )
  | VecRef (a, i) -> (
    let vec_ty = type_check_atm ty_env a in
    match vec_ty with
    ...
    | Some (Vector vec) ->
      if i < 0 || i >= Array.length vec then
        ty_err_simple
        @@ Printf.sprintf
             ("type_check_exp: " ^^ "vector-ref : index %d is out of range")
             i
      else Some vec.(i)
    | Some t ->
      ty_err_simple
      @@ Printf.sprintf
           ("type_check_exp: " ^^ "vector-ref : expected a vector, got: %s")
           (string_of_ty t) )
  | VecSet (a1, i, a2) -> (
    ...
    | Some (Vector vec) -> (
      if i < 0 || i >= Array.length vec then
        ty_err_simple
        @@ Printf.sprintf
             ("type_check_exp: " ^^ "vector-set! : index %d is out of range")
             i
      else
    ...
    | Some t ->
      ty_err_simple
      @@ Printf.sprintf
           ("type_check_exp: " ^^ "vector-set! : expected a vector, got: %s")
           (string_of_ty t) )
  | FunRef (Label lbl, i) -> (
    ...
    | None ->
      ty_err_simple
      @@ Printf.sprintf
           ("type_check_exp: " ^^ "fun-ref : no binding for function %s")
           lbl
    | Some ty -> (
      ...
      match ty with
      | Function (arg_tys, _) ->
        if List.length arg_tys = i then Some ty
        else
          ty_err_simple
          @@ Printf.sprintf
               ("type_check_exp: " ^^ "fun-ref : wrong function arity for %s")
               lbl
      | _ ->
        ty_err_simple
        @@ Printf.sprintf
             ("type_check_exp: " ^^ "fun-ref : non-function type for %s")
             lbl ) )
  | Call (atm, atms) -> type_check_call ty_env atm atms
...
let type_check_stmt (ty_env : ty_env_t) (s : stmt) : ty_env_t =
  ...
  | PrimS (`Read, []) -> ty_env
  | PrimS (`Read, _) ->
    ty_err_simple ("type_check_stmt: " ^ "read : wrong number of arguments")
  | PrimS (`Print, [a]) -> (
  ...
  | PrimS (`Print, _) ->
    ty_err_simple ("type_check_stmt: " ^ "print: wrong number of arguments")
  | CallS (atm, atms) ->
  ...
  | VecSetS (a1, i, a2) -> (
    ...
    | Some (Vector vec) -> (
      if i < 0 || i >= Array.length vec then
        ty_err_simple
        @@ Printf.sprintf
             ("type_check_stmt: " ^^ "vector-set! : index %d is out of range")
             i
      else
        match type_check_atm ty_env a2 with
        | None -> ty_env
    ...
    | Some t ->
      ty_err_simple
      @@ Printf.sprintf
           ("type_check_stmt: " ^^ "vector-set! : expected a vector, got: %s")
           (string_of_ty t) )
...
let rec type_check_tail (ty_env : ty_env_t) (t : tail) : ty_env_t =
  ...
  | IfStmt {op; arg1; arg2; _} -> (
    match type_check_exp ty_env (Prim ((op :> core_op), [arg1; arg2])) with
    | None
     |Some Boolean ->
      ty_env
...
let type_check_tail_return (ty_env : ty_env_t) (t : tail) : ty option =
  ...
  | Return e -> (
    match type_check_exp ty_env e with
    | None -> ty_err_simple "type_check_tail_return: return with no type"
    | Some t -> Some t )
  | TailCall (atm, atms) -> type_check_call ty_env atm atms
  | Goto _
   |Seq _
   |IfStmt _ ->
    None
...
let compute_ty_env (ty_env : ty_env_t) (lmap : tail LabelMap.t) : ty_env_t =
...
let get_locals_types (vlst : var list) (ty_env : ty_env_t) : (var * ty) list =
  ...
  VarSet.fold
    (fun v map ->
      ...
      | None ->
        ty_err_simple
        @@ Printf.sprintf "get_locals_types: variable (%s) has no type binding"
             v
      | Some ty -> VarMap.add v ty map )
    vset' VarMap.empty
  |> VarMap.bindings
...
let type_check_def (ty_env : ty_env_t) (d : def) : def =
  ...
  if List.for_all (fun ret_ty -> ret_ty = fc.ret) ret_tys then
    ...
    Def (lbl, fc')
  else failwithf "invalid return type(s) for function %s" (string_of_label lbl)
...
let ty_of_fun_contents name {args; ret; _} =
  ...
  if no_string_repeats formals then Function (ts, ret)
    ...
    failwithf "cfun: repeated formal arguments in function %s: (%s)" name
      formals_string
