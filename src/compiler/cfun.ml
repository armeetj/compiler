...
type program = CProgram of def list [@@deriving sexp]
...
let get_vars (body : (label * tail) list) : var list =
  ...
  let vars_in_exp = function
    | Atm a -> vars_in_atm a
    | Prim (_, atms) -> List.concat_map vars_in_atm atms
    | Allocate _ -> []
    | GlobalVal v -> [v]
    | VecLen a -> vars_in_atm a
    | VecRef (a, _) -> vars_in_atm a
    | VecSet (a1, _, a2) -> vars_in_atm a1 @ vars_in_atm a2
    | FunRef (Label lbl, _) -> [lbl]
    | Call (a, atms) -> vars_in_atm a @ List.concat_map vars_in_atm atms
  in
  let vars_in_stmt = function
    | Assign (v, e) -> add_vars (vars_in_exp e) [v]
    | PrimS (_, atms) -> List.concat_map vars_in_atm atms
    | CallS (atm, atms) -> vars_in_atm atm @ List.concat_map vars_in_atm atms
    | Collect _ -> []
    | VecSetS (a1, _, a2) -> vars_in_atm a1 @ vars_in_atm a2
  in
  let rec vars_in_tail t =
    match t with
    | Return e -> vars_in_exp e
    | TailCall (a, atms) -> vars_in_atm a @ List.concat_map vars_in_atm atms
    | Seq (s, t) -> add_vars (vars_in_stmt s) (vars_in_tail t)
    | Goto _ -> []
    | IfStmt {arg1; arg2; _} -> add_vars (vars_in_atm arg1) (vars_in_atm arg2)
  in
