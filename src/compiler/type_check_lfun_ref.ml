...
let rec type_check_exp (ty_env : ty_env_t) (e : exp) : exp * ty =
  ...
  | FunRef (Label v, _) ->
    let t =
      Env.find_or v ty_env ~f:(fun () ->
        ty_err_simple ("unbound function: " ^ v) )
    in
    (e, t)
  | Prim (op, es) ->
  ...
  | Vec (es, topt) -> (
    ...
    if len > 50 then
      ...
      | Some t' ->
        if ty_equal t t' then (Vec (es', topt), t)
        else
          ty_err_simple
          @@ Printf.sprintf
  ...
  | VecSet (e1, i, e2) -> (
    ...
    | Vector ts ->
      let len = Array.length ts in
      if i >= 0 && i < len then
        if ty_equal ts.(i) t2 then (VecSet (e1', i, e2'), t2)
        else ty_err "vector-set!" ~expected:ts.(i) ~got:t2
      else
        ty_err_simple
...
let type_check_def (ty_env : ty_env_t) (d : def) : def =
  ...
  if ty_equal ret_ty fc.ret then Def (Label v, {fc with body = body'})
  else
    ty_err
